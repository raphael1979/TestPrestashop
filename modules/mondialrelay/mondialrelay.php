...
class MondialRelay extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->includeFiles($this->getLocalPath().DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR);
        $this->includeFiles($this->getLocalPath().DIRECTORY_SEPARATOR.'controllers'.DIRECTORY_SEPARATOR);
     ...
     * @param        $path
     * @param string $call_user_func
     * @param array  $params
     * @return bool
     */
    private function includeFiles($path, $call_user_func = null, $params = array())
    {
        $not_include_these_files = array(
            'lib',
            'test.php',
        );
        ...
        if (is_dir($path)) {
            ...
            if ($files && is_array($files) && count($files) > 2) {
                ...
                foreach ($files as $file) {
                    if (in_array($file, $not_include_these_files)) {
                        continue;
                    }

                    # Check if files is not "." or ".." or "*.old*" or "index.php"
                    if (!preg_match('#^\.+$#', $file) && !preg_match('#\.old#isD', $file) && $file != "index.php") {
                        # If file is a real file ".php"
                        if (strrchr($file, '.') == '.php' && is_file($path.$file)) {
                            # Get full file name
                            $file_name = Tools::substr($file, 0, -4);

                            # Check if class has already loaded
                            if (!class_exists($file_name)) {
                                require_once $path.$file_name.'.php';
                            }

                            # If call user fund and method exists
                            if ($call_user_func && method_exists($file_name, $call_user_func)) {
                                # Apply the method
                                if (!call_user_func_array(array($file_name, $call_user_func), $params)) {
                                    return false;
                                }
                            }
                        } elseif (is_dir($path.$file.DIRECTORY_SEPARATOR)) { # If file is a folder
                            # Include files
                            if (!$this->includeFiles($path.$file.DIRECTORY_SEPARATOR)) {
                                return false;
                            }
                        }
                    }
    ...
    {
        MRLogger::getInstance()->info('methode installTabs de MondialRelay');
        ...
        return $this->includeFiles($path, 'install', $datas);
    }
    ...
    */
    ...
    {
        if ((!$this->registerHook('displayCarrierExtraContent') || !$this->registerHook('updateCarrier')
    ...
    {
        if (!MRAdminTabHelper::deleteAdminTabs($this->name)) {
            return false;
        }
    ...
    */
    ...
    public function hookDisplayOrderDetail($params)
    {
        ...
        $point_relais_addr1 = $res['MR_Selected_LgAdr1'] ? $res['MR_Selected_LgAdr1'] : ' ';
        $point_relais_addr2 = $res['MR_Selected_LgAdr2'] ? $res['MR_Selected_LgAdr2'] : ' ';
        $point_relais_addr3 = $res['MR_Selected_LgAdr3'] ? $res['MR_Selected_LgAdr3'] : ' ';
        $point_relais_addr4 = $res['MR_Selected_LgAdr4'] ? $res['MR_Selected_LgAdr4'] : ' ';
        $point_relais_cp = $res['MR_Selected_CP'] ? $res['MR_Selected_CP'] : ' ';
        $point_relais_city = $res['MR_Selected_Ville'] ? $res['MR_Selected_Ville'] : ' ';
        $point_relais_country = $res['MR_Selected_Pays'] ? $res['MR_Selected_Pays'] : ' ';
        ...
        if (($res['dlv_mode'] == 'LD1') || ($res['dlv_mode'] == 'LDS') || ($res['dlv_mode'] == 'HOM')) {
            ...
            $this->context->smarty->assign(
                array(
                    'mr_addr1' => $point_relais_addr1,
                    'mr_addr2' => $point_relais_addr2,
                    'mr_addr3' => $point_relais_addr3,
                    'mr_addr4' => $point_relais_addr4,
                    'mr_cp' => $point_relais_cp,
                    'mr_city' => $point_relais_city,
                    'mr_country' => $point_relais_country,
                    'mr_url'  => $res['url_suivi'],
                )
            );
        }
        $this->context->controller->registerJavascript('module-'.$this->name.'-common', $this->_path.'views/js/order_detail.js');
        echo $this->fetch('module:mondialrelay/views/templates/front/order_detail.tpl');
    }
    ...
    */
        ...
        if (in_array($file, array('order-opc', 'order', 'orderopc'))) {
            $this->context->smarty->assign(
                array(
                    ...
                    'order_url' => str_replace('/index.php', '', $_SERVER['SCRIPT_URI']).$_SERVER['REQUEST_URI'],
            ...
            if (Configuration::get('MONDIAL_RELAY_MODE') == 'widget') {
                ...
                if (isset($this->context) && method_exists($this->context->controller, 'addJS')) {
                    $this->context->controller->addJqueryPlugin('fancybox');
                    $this->context->controller->registerJavascript('module-'.$this->name.'-common', $this->_path.'views/js/common.js');
                    $this->context->controller->registerJavascript('module-'.$this->name.'-mondialrelay', $this->_path.'views/js/mondialrelay.js');
                    $this->context->controller->registerJavascript('module-'.$this->name.'-gmaps', $this->_path.'views/js/gmap.js');
                }
    ...
    public function hookDisplayCarrierExtraContent($params)
    {
        ...
        foreach ($carriersList as $k => $row) {
            ...
            if (method_exists($this->context->cart, 'carrierIsSelected')) {
                if ($this->context->cart->carrierIsSelected($row['id_carrier'], $this->context->cart->id_address_delivery)) {
        ...
        $weight = $this->context->cart->getTotalWeight() * $this->account_shop['MR_WEIGHT_COEFFICIENT'];
     ...
     * Fonction de log
     *
     * Enregistre dans /modules/mondialrelay/debug.log
     *
     * @param     $object
    ...
    public static function debug($object, $error_level = 0, $advanced_log = false)
    {
        ...
        if ($advanced_log) {
            ...
            if (count($backtrace) > 3) {
                $file .= $backtrace[3]['class'].'::'.$backtrace[3]['function'].' -> '.$backtrace[2]['class'].'::'
                    .$backtrace[2]['function'].' -> '.$backtrace[1]['class'].'::'.$backtrace[1]['function'].' -> '
                    .$backtrace[0]['class'].'::'.$backtrace[0]['function'];
            } elseif (count($backtrace) > 2) {
                $file .= $backtrace[2]['function'].' -> '.$backtrace[1]['class'].'::'.$backtrace[1]['function'].' -> '
                    .$backtrace[0]['class'].'::'.$backtrace[0]['function'];
            } elseif (count($backtrace) > 1) {
                $file .= $backtrace[1]['class'].'::'.$backtrace[1]['function'].' -> '.$backtrace[0]['class'].'::'
        ...
        $stderr = fopen(_PS_MODULE_DIR_.'/'.$module_name.'/debug.log', 'a');
